...
var DrawingCanvas = (function DrawingCanvasClosure() {
  ...
  function DrawingCanvas(options) {
    ...
    this.ruler_index = 32; // 자의 최소 길이 : 30 = 3cm(30mm), 맨 앞,뒤에 1mm 여유를 위해 +2해줌
  ...
  DrawingCanvas.prototype = {
    ...
    createStage: function DrawingCanvas_createStage() {
      ...
      this.mathToolLayer = new Konva.Layer({
        id: "mathTool",
        name: "mathTool_layer",
      });
    ...
    clear: function DrawingCanvas_clear() {
      ...
      window.dispatchEvent(
        new CustomEvent("DrawingCanvasEvent", {
          type: "script",
          detail: "clearDrawing();",
        })
      );
    ...
    drawline: function DrawingCanvas_drawline(x, y) {
      if (this.mode == "brush" || this.mode == "eraser") {
        ...
        if (!this.drawingLine) {
          ...
          var konva_newPoints = this.drawingLine
            .points()
            .concat([x / this.scale, y / this.scale]);
          this.drawingLine.points(konva_newPoints);
        }
        this.drawingLayer.batchDraw();
      }
    },
    drawlineWithDetail: function DrawingCanvas_drawlineWithDetail(
      x,
      y,
      mode,
      stroke,
    ...
    drawLineStroke_change: function DrawingCanvas_drawLineStroke_change(
      stroke
    ) {
      this.stroke = stroke;
    },
    drawLineStrokeWidth_change:
      function DrawingCanvas_drawLineStrokeWidth_change(strokeWidth) {
        this.strokeWidth = strokeWidth;
      },
    ...
    _setTransformer: function DrawingCanvas_setTransformer(e) {
      ...
      if (this.mode == "text" && e.target.attrs.name == "textNodes") {
        _transformer = new Konva.Transformer({
          ...
          boundBoxFunc: function (oldBox, newBox) {
            var max_width =
              (e.target.parent.parent.attrs.width - newBox.x) * 0.9;
            if (
              Math.abs(newBox.width) < 100 ||
              Math.abs(newBox.width) > max_width
            ) {
        ...
        _transformer = new Konva.Transformer({
          ...
          boundBoxFunc: function (oldBox, newBox) {
            ...
            if (
              Math.abs(newBox.width) <= 100 ||
              Math.abs(newBox.width) >= 500
            ) {
              if (Math.abs(newBox.width) <= 100) {
                _transformer.stopTransform();
                e.target.attrs.width = 110;
                e.target.attrs.height = 110;
        ...
        _transformer = new Konva.Transformer({
          ...
          boundBoxFunc: function (oldBox, newBox) {
            if (
              Math.abs(newBox.width) <= 200 ||
              Math.abs(newBox.width) >= 600
            ) {
        ...
        this.mode == "mathTool" &&
        e.target.attrs.name == "protractor"
      ) {
        _transformer = new Konva.Transformer({
          rotationSnaps: [0, 90, 180, 270],
          nodes: [e.target],
      ...
      if (!_deletebutton) {
        if (this.mode == "text" && e.target.attrs.name == "textNodes") {
          ...
          this.mode == "image" &&
          e.target.attrs.name == "imageNodes"
        ) {
          _deletebutton = new Konva.Group({
            x: e.target.getWidth() + 3.5,
            y: -17,
          ...
          if (!e.target.attrs.width) {
            ...
            get_width =
              e.target.getWidth() +
              e.target.attrs.strokeWidth * e.target.scaleX();
          } else {
        ...
        if (
          _target_url == "preview" ||
          $("#drawing-button").hasClass("active")
        ) {
          deleteicon.src = "img/viewer/tr_delete.png";
        } else {
        ...
        if (this.mode == "text") {
          ...
          if (e.target.attrs.name == "ruler") {
            ...
            _transformer.on("transform", function () {
              var node_width =
                (_transformer.width() + 10) * _transformer.scaleX();
              _transformer.setAttrs({ width: node_width });
              _deletebutton.x(node_width);
    ...
    _onclick: function DrawingCanvas_onclick(e) {
      ...
      if (this.mode == "text" && e.target.attrs.name == "textNodes") {
        ...
        e.target.on("dragmove touchmove transform", function (e) {
        ...
        e.target
          .off("dragmove touchmove transform")
          .on("dragmove touchmove transform", function (e) {
            var stage_width = $(".konvajs-content").width();
            var stage_height = $(".konvajs-content").height();
            var box = e.target.getClientRect();
            var absPos = e.target.getAbsolutePosition();
    ...
    _onmousedown: function DrawingCanvas_onmousedown(e) {
      ...
      if (
        ...
        var pos = this.stage.getPointerPosition();
        this.make_default_arrowWithDetail(
          pos.x,
          pos.y,
          this.mode,
          this.arrow_fill,
    ...
    sendTransformSticker: function DrawingCanvas_sendTransformSticker(
      id,
      width
    ) {
      var self = this;
    ...
    sendTransformNode: function DrawingCanvas_sendTransformNode(
      id,
      type,
      width,
      height,
    ...
    textwrite: function DrawingCanvas_textwrite() {
      ...
      if (this.first_make_textarea) {
        ...
        if (
          _points.x > board_width - board_width * 0.2 ||
          _points.y > board_height - board_height * 0.2
        ) {
          if (
            _points.x > board_width - board_width * 0.2 &&
            _points.y > board_height - board_height * 0.2
      ...
      textNode.on("dblclick dbltap", function (e) {
        ...
        var stageBox = _target.parent.parent
          .container()
        ...
        if (self.type == "BOARD") {
          var win_board_get_scale =
              /matrix\((-?\d*\.?\d+),\s*0,\s*0,\s*(-?\d*\.?\d+),\s*0,\s*0\)/,
            matches = $("#board_wrap")
              .css("transform")
              .match(win_board_get_scale);
          ...
          if (stage_parent.attr("id") == "drawing-control") {
            ...
            var iframe_get_scale =
                /matrix\((-?\d*\.?\d+),\s*0,\s*0,\s*(-?\d*\.?\d+),\s*0,\s*0\)/,
              iframe_matches = stage_parent
                .parent()
                .css("transform")
                .match(iframe_get_scale);
        ...
        if (self.type == "BOARD") {
          ...
          textarea.style.maxWidth =
            self.stage.attrs.width - areaPosition.x + "px";
          textarea.style.maxheight =
            self.stage.attrs.height - areaPosition.y + "px";
        } else {
          textarea.style.top = areaPosition.y * _get_parent_scaleY + "px";
          textarea.style.left = areaPosition.x * _get_parent_scaleX + "px";
          textarea.style.width =
            self.stage.attrs.width * _get_parent_scaleX + "px";
          textarea.style.height = _target.height() * _get_parent_scaleY + "px";
          textarea.style.maxWidth =
            (self.stage.attrs.width - textPosition.x) * _get_parent_scaleX +
            "px";
          textarea.style.maxheight =
            (self.stage.attrs.height - textPosition.y) * _get_parent_scaleY +
            "px";
        ...
        // maxRows
        textarea.rows = Math.floor(
          parseInt(_target.parent.parent.attrs.height) /
            parseInt(_target.attrs.fontSize)
        );
        ...
        function removeTextarea() {
          ...
          _target.show();
          if (_target.parent) _target.parent.draw();
        }
        ...
        textarea.addEventListener("keydown", function (e) {
          ...
          var _cur_canvas_height =
            parseInt(_canvas_height) - parseInt(_canvas_parent_top);
          var _textarea_top = textarea.style.top;
          var _cur_height =
            parseInt(_textarea_top) + parseInt(textarea.style.height);
          ...
          if (
            e.keyCode === 13 &&
            (numberOfLines >= maxRow || _cur_height >= _cur_canvas_height)
          ) {
            e.preventDefault();
            return false;
          }
        ...
        function handleOutsideClick(e) {
          if (
            ...
            if (
              _target.text() === "" ||
              _target.attrs.hasOwnProperty("text") == false
            ) {
              _target.destroy();
            }
    ...
    send_text_fontFamily_change:
      function DrawingCanvas_send_text_fontFamily_change(id, fontFamily) {
        var self = this;
    ...
    text_fontFamily_change: function DrawingCanvas_text_fontFamily_change(
      change_fontFamily
    ) {
      var self = this;
      ...
      for (var i = 0; i < textNode_langth; i++) {
        ...
        if (this.textLayer.find(".textNodes")[i].attrs.draggable) {
          this.textLayer.find(".textNodes")[i].attrs.fontFamily =
            str_font_family;
    ...
    send_text_fontSize_change: function DrawingCanvas_send_text_fontSize_change(
      id,
      fontSize
    ) {
      var self = this;
    ...
    text_fontSize_change: function DrawingCanvas_text_fontSize_change(
      change_fontSize
    ) {
      var self = this;
      var text_layer = this.textLayer;
    ...
    send_text_font_size_lg: function DrawingCanvas_text_font_size_lg(
      id,
      fontSize
    ) {
      var self = this;
    ...
    text_font_size_lg: function DrawingCanvas_text_font_size_lg(
      change_fontSize
    ) {
      var self = this;
      var text_layer = this.textLayer;
    ...
    send_text_font_size_sm: function DrawingCanvas_text_font_size_sm(
      id,
      fontSize
    ) {
      var self = this;
    ...
    text_font_size_sm: function DrawingCanvas_text_font_size_sm(
      change_fontSize
    ) {
      var self = this;
      var text_layer = this.textLayer;
        ...
        if (this.textLayer.find(".textNodes")[i].attrs.draggable) {
          ...
          window.dispatchEvent(
            new CustomEvent("DrawingCanvasEvent", {
              type: "script",
              detail:
                "send_text_font_fill('" + _id + "', '" + change_fill + "');",
            })
          );
        }
      }
    ...
    drawHlighterWithDetail: function DrawingCanvas_drawHlighterWithDetail(
      x,
      y,
      mode,
      stroke,
    ...
    select_change_hlighter_color:
      function DrawingCanvas_select_change_hlighter_color(change_color) {
        ...
        for (var i = 0; i < hlighter_langth; i++) {
          if (this.imageLayer.find(".hlighter")[i].attrs.draggable) {
            this.imageLayer.find(".hlighter")[i].attrs.stroke =
              this.hlighter_color;
            ...
            window.dispatchEvent(
              new CustomEvent("DrawingCanvasEvent", {
                type: "script",
                detail:
                  "send_select_change_hlighter_color('" +
                  _id +
                  "', '" +
                  change_color +
                  "');",
    ...
    send_select_change_hlighter_color:
      function DrawingCanvas_send_select_change_hlighter_color(id, color) {
        var self = this;
    ...
    select_change_hlighter_width:
      function DrawingCanvas_select_change_hlighter_width(change_width) {
        this.hlighter_width = parseInt(change_width);

        var hlighter_langth = this.stage.find(".hlighter").length;
        for (var i = 0; i < hlighter_langth; i++) {
          if (this.imageLayer.find(".hlighter")[i].attrs.draggable) {
            this.imageLayer.find(".hlighter")[i].attrs.strokeWidth =
              this.hlighter_width;
            ...
            window.dispatchEvent(
              new CustomEvent("DrawingCanvasEvent", {
                type: "script",
                detail:
                  "send_select_change_hlighter_width('" +
                  _id +
                  "', " +
                  this.hlighter_width +
                  ");",
    ...
    send_select_change_hlighter_width:
      function send_select_change_hlighter_width(id, width) {
        var self = this;
    ...
    send_arrow_color_change: function DrawingCanvas_send_arrow_color_change(
      id,
      color
    ) {
      var self = this;
    ...
    arrow_direction_change: function DrawingCanvas_arrow_direction_change(
      direction
    ) {
      ...
      for (var i = 0; i < arrow_langth; i++) {
        ...
        if (this.imageLayer.find(".arrow")[i].attrs.draggable) {
          if (direction == "bidirectional") {
            this.imageLayer.find(".arrow")[i].attrs.points =
              reverse_points.concat([
                reverse_points[0],
                reverse_points[1],
                reverse_points[2],
                reverse_points[3],
              ]);
            this.imageLayer.find(".arrow")[i].attrs.pointerAtBeginning =
              this.pointerAtBeginning;
          } else {
            this.imageLayer.find(".arrow")[i].attrs.pointerAtBeginning =
              this.pointerAtBeginning;
    ...
    send_arrow_direction_change:
      function DrawingCanvas_send_arrow_direction_change(
        id,
        points,
        pointerAtBeginning
      ) {
    ...
    arrow_width_change: function DrawingCanvas_arrow_width_change(arrow_width) {
      ...
      for (var i = 0; i < arrow_langth; i++) {
        if (this.imageLayer.find(".arrow")[i].attrs.draggable) {
          this.imageLayer.find(".arrow")[i].attrs.strokeWidth =
            parseInt(arrow_width);
          ...
          window.dispatchEvent(
            new CustomEvent("DrawingCanvasEvent", {
              type: "script",
              detail:
                "send_arrow_width_change('" + _id + "'," + arrow_width + ");",
            })
          );
        }
      }
    },
    send_arrow_width_change: function DrawingCanvas_send_arrow_width_change(
      id,
      width
    ) {
      var self = this;
        ...
        if (this.imageLayer.find(".arrow")[i].attrs.draggable) {
          ...
          window.dispatchEvent(
            new CustomEvent("DrawingCanvasEvent", {
              type: "script",
              detail:
                "send_arrow_dash_change('" + _id + "',[" + _arrow_dash + "]);",
            })
          );
        }
      }
    },
    send_arrow_dash_change: function DrawingCanvas_send_arrow_dash_change(
      id,
      dash
    ) {
      var self = this;
    ...
    make_default_arrow: function DrawingCanvas_make_default_arrow(x, y) {
      ...
      if (this.mode == "arrow") {
        ...
        if (this.arrow_direction == "reverse") {
          ...
          this.drawarrowLine.points(
            this.drawarrowLine
              .points()
              .concat([x, y, reverse_points[0], reverse_points[1]])
          );
        }
    ...
    make_default_arrowWithDetail:
      function DrawingCanvas_make_default_arrowWithDetail(
        x,
        y,
        mode,
        fill,
    ...
    make_lineWithDetail: function DrawingCanvas_make_lineWithDetail(
      x,
      y,
      mode,
      stroke,
    ...
    line_color_change: function DrawingCanvas_line_color_change(line_color) {
      ...
      for (var i = 0; i < line_langth; i++) {
        if (this.imageLayer.find(".straight")[i].attrs.draggable) {
          this.imageLayer.find(".straight")[i].attrs.stroke =
            this.straight_color;
          ...
          window.dispatchEvent(
            new CustomEvent("DrawingCanvasEvent", {
              type: "script",
              detail:
                "send_line_color_change('" + _id + "', '" + line_color + "');",
            })
          );
        }
      }
    },
    send_line_color_change: function DrawingCanvas_send_line_color_change(
      id,
      color
    ) {
      var self = this;
    ...
    line_width_change: function DrawingCanvas_line_width_change(line_width) {
      ...
      for (var i = 0; i < line_langth; i++) {
        if (this.imageLayer.find(".straight")[i].attrs.draggable) {
          this.imageLayer.find(".straight")[i].attrs.strokeWidth =
            this.straight_width;
          ...
          window.dispatchEvent(
            new CustomEvent("DrawingCanvasEvent", {
              type: "script",
              detail:
                "send_line_width_change('" + _id + "', '" + line_width + "');",
            })
          );
        }
      }
    },
    send_line_width_change: function DrawingCanvas_send_line_width_change(
      id,
      width
    ) {
      var self = this;
        ...
        if (this.imageLayer.find(".straight")[i].attrs.draggable) {
          ...
          window.dispatchEvent(
            new CustomEvent("DrawingCanvasEvent", {
              type: "script",
              detail:
                "send_line_dash_change('" + _id + "', [" + _line_dash + "]);",
            })
          );
        }
      }
    },
    send_line_dash_change: function DrawingCanvas_send_line_dash_change(
      id,
      dash
    ) {
      var self = this;
          ...
          textNode.on("dblclick", function (e) {
            ...
            if (self.type == "BOARD") {
              ...
              textarea.style.height =
                _target.height() * _get_parent_scaleY + "px";
              textarea.style.maxWidth =
                (self.stage.attrs.width - textPosition.x) * _get_parent_scaleX +
                "px";
              textarea.style.maxheight =
                (self.stage.attrs.height - textPosition.y) *
                  _get_parent_scaleY +
                "px";
              // textarea.style.maxWidth = ((self.stage.attrs.width - areaPosition.x) * _get_parent_scaleX) + 'px';
              // textarea.style.maxheight = ((self.stage.attrs.height - areaPosition.y) * _get_parent_scaleY) + 'px';
            ...
            if (_target.text() != "") {
              textarea.style.width =
                _target.width() * _get_parent_scaleX + "px";
            }
            ...
            textarea.addEventListener("keydown", function (e) {
              ...
              var maxRow = textarea.rows;
              var _canvas_height = $(".konvajs-content").height();
              var _canvas_parent_top = $(".konvajs-content")
                .parent()
                .css("top");
              var _cur_canvas_height =
                parseInt(_canvas_height) - parseInt(_canvas_parent_top);
                ...
                if (target_parent) {
                  ...
                  window.dispatchEvent(
                    new CustomEvent("DrawingCanvasEvent", {
                      type: "script",
                      detail:
                        "send_text_node(" + JSON.stringify(_target) + ");",
                    })
                  );
                }
              }
    ...
    create_set_square: function () {
      ...
      var self = this;
      var url = "./img/viewer/tri-ruler-trans.png";
    ...
    create_protractor: function () {
      ...
      var self = this;
      var url = "./img/viewer/protractor.png";
    ...
    get_dataRatio: function () {
      ...
      // 자를 사용하지 않는 페이지에서는 5.0으로 처리
      if ($("#iframe_content").contents().find("div#wrap").data("ratio")) {
        _ratio = $("#iframe_content").contents().find("div#wrap").data("ratio");
      } else {
        _ratio = 5.0;
      }
    ...
    create_ruler_box: function () {
      ...
      var grad_wrap = new Konva.Rect({
        x: 100,
        y: 100,
        width: this.tool_ratio * this.ruler_index,
        height: 100,
        id: "grad_wrap",
        name: "ruler",
        scaleX: this.ruler_scale_x,
      ...
      this.ruler_group.add(grad_wrap);
    ...
    node_remove_keydown_event: function (e) {},
    make_uuid: function DrawingCanvas_make_uuid() {
      this.node_id = undefined;

      this.node_id = "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(
        /[xy]/g,
        function (c) {
